...
chromium-browser --no-sandbox --kiosk "about:blank" &
...
if kill -0 $XORG_PID 2>/dev/null; then
    ...
    sleep 3
    CHROMIUM_PID=$(pgrep -f chromium-browser || echo "")
    ...
    if [ -n "$CHROMIUM_PID" ]; then
        ...
        DISPLAY=:0 chromium-browser --no-sandbox --kiosk "about:blank" > /var/log/container/chromium.log 2>&1 &
